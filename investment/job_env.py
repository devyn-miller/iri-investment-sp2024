...
def common_job_str(job, current=True):
    ...
    if job.is_ended():
        if h_deadline <= 0:
            ...
            deadline_str = f"{1-s_deadline}天延迟"
        else:
            deadline_str = "按时完成"
    else:
        if h_deadline == 1:
            ...
            deadline_str = f"0/{h_deadline} 天"
        else:
            deadline_str = f"{max(0,s_deadline):2}/{h_deadline} 天"
    length_str = f"{job.expected_length(current):2.1f} 天 [{job.lower_length(current)}--{job.upper_length(current)}]"
